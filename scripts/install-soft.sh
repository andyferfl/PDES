...
make --jobs=$(nproc) >> $INSTALL_LOGS_DIR 2>/dev/null
make check >> $INSTALL_LOGS_DIR 2>/dev/null

echo "Installing openmpi..."
make install >> $INSTALL_LOGS_DIR 2>/dev/null

echo "Decompressing libtimestretch..."
tar -xvzf $SOFT_DIR/libtimestretch-master.tar.gz --directory=$SOFT_DIR >> $INSTALL_LOGS_DIR 2>/dev/null
...
echo "Dependencies for SST installed."
